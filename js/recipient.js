...
(function () {
  const { el, $, $$, toast, sheet, detectType, fileToBase64, fileToArrayBuffer, downloadBase64, MIME, confirmDialog, iconHTML, fileIcon } = U;
  const t = (k) => I18N.t(k);
  ...
  Screens.send = function (main) {
    ...
    const sources = [];
    const drop = el('div', { class: 'card center', style: { padding: '30px 16px', cursor: 'pointer', border: '1.5px dashed var(--line-2)' } }, [
      el('div', { style: { width: '64px', height: '64px', borderRadius: '18px', background: 'var(--brand-50)', color: 'var(--brand)', display: 'grid', placeItems: 'center', margin: '0 auto 12px' }, html: iconHTML('send', 28) }),
    ...
    const cont = el('button', { class: 'btn primary', style: { display: 'none', marginTop: '6px' }, html: t('se_edit') + iconHTML('arrow-right', 18) });
    ...
    function renderList() {
      ...
      sources.forEach((s, i) => {
        card.appendChild(el('div', { class: 'row' }, [
          el('div', { class: 'ic file' }, [fileIcon(s.type, 38)]),
          el('div', { class: 'main' }, [el('div', { class: 't', text: s.file.name }), el('div', { class: 'sub', text: s.type.toUpperCase() + ' · ' + Math.ceil(s.file.size / 1024) + ' KB' })]),
          el('div', { class: 'actions' }, [el('button', { class: 'icon-btn', html: iconHTML('x', 18), onclick: () => { sources.splice(i, 1); renderList(); } })])
        ]));
      });
  ...
  const TOOLS = [
    ['sign', 'tool_sign', 'signature'],
    ['text', 'tool_text', 'type'],
    ['date', 'tool_date', 'calendar'],
    ['check', 'tool_check', 'check-square'],
  ];
  const DEF = { sign: [26, 9], text: [24, 5], date: [18, 5], check: [7, 5] };
  const FIELD_ICON = { sign: 'signature', text: 'type', date: 'calendar', check: 'check-square' };

  Screens.editor = function (main) {
    const doc = window.SendDoc;
    if (!doc || !doc.sources || !doc.sources.length) { go('send'); return; }

    let activeTool = null, scale = 1, selected = null;
    const wrap = el('div', { class: 'editor-wrap' });

    const toolbar = el('div', { class: 'editor-toolbar' });
    toolbar.appendChild(el('button', { class: 'icon-btn tbar-back', html: iconHTML(I18N.dir() === 'rtl' ? 'arrow-right' : 'arrow-left', 20), onclick: () => go('send') }));
    toolbar.appendChild(el('span', { class: 'tbar-sep' }));
    const toolBtns = {};
    TOOLS.forEach(([key, label, ic]) => {
      const b = el('button', { class: 'tool', html: iconHTML(ic, 20) + '<span>' + t(label) + '</span>', onclick: () => selectTool(key) });
      toolBtns[key] = b; toolbar.appendChild(b);
    });
    wrap.appendChild(toolbar);
    wrap.appendChild(el('div', { class: 'banner info', style: { margin: '8px 10px 0', fontSize: '12px' }, html: iconHTML('info', 16) + t('tool_hint') }));
    ...
    footer.appendChild(el('div', { class: 'zoom-ctl' }, [
      ...
      el('span', { id: 'zlevel', class: 'zlevel', text: '100%' }),
      el('button', { html: '+', onclick: () => setZoom(scale + 0.2) }),
    ]));
    footer.appendChild(el('button', {
      class: 'btn primary', html: t('ed_continue') + iconHTML('arrow-right', 18),
    ...
    const pageEls = [];
    buildPages(stack, doc.sources, () => { (doc.fields || []).forEach(f => mountField(f)); refreshFonts(); });

    async function buildPages(stackEl, sources, done) {
      const baseW = Math.max(280, (canvas.clientWidth || PAGE_FALLBACK_W) - 28);
      for (const src of sources) {
        if (src.type === 'pdf' && window.pdfjsLib) { try { await renderPdf(src, baseW, stackEl); continue; } catch (e) { console.warn(e); } }
        if (src.type === 'docx' && window.mammoth) { try { await renderDocx(src, baseW, stackEl); continue; } catch (e) { console.warn(e); } }
    ...
    function placeholderPage(src, baseW, stackEl) {
      const pageEl = el('div', { class: 'pdf-page ph', style: { width: baseW + 'px', height: Math.round(baseW * 1.414) + 'px' } }, [
        el('div', { class: 'ph-body' }, [fileIcon(src.type, 54), el('div', { class: 't', text: src.file.name }), el('p', { class: 'small muted', text: t('se_edit') })])
      ]);
      stackEl.appendChild(pageEl);
      registerPage(pageEl);
    }
    ...
    function addField(type, pageIdx, xPct, yPct) {
      const [w, h] = DEF[type];
      const f = { id: U.uid(), type, page: pageIdx, x: clamp(xPct - w / 2, 0, 100 - w), y: clamp(yPct - h / 2, 0, 100 - h), w, h };
      doc.fields.push(f);
      mountField(f, true);
    }
    function labelHTML(type) {
      const txt = type === 'sign' ? t('tool_sign') : type === 'date' ? t('tool_date') : type === 'check' ? '' : t('tool_text');
      return iconHTML(FIELD_ICON[type], 13) + (txt ? '<span>' + txt + '</span>' : '');
    }
    function mountField(f, select) {
      const pageEl = pageEls[f.page]; if (!pageEl) return;
      const node = el('div', { class: 'fld', 'data-id': f.id, style: { left: f.x + '%', top: f.y + '%', width: f.w + '%', height: f.h + '%' } }, [
        el('span', { class: 'lbl', html: labelHTML(f.type) }),
        el('button', { class: 'del', html: iconHTML('x', 13), onclick: (e) => { e.stopPropagation(); removeField(f, node); } }),
    ...
    function removeField(f, node) { const i = doc.fields.indexOf(f); if (i >= 0) doc.fields.splice(i, 1); node.remove(); if (selected === f) selected = null; }
    function selectField(f) { selected = f; $$('.fld', stack).forEach(n => n.classList.remove('sel')); f._node && f._node.classList.add('sel'); }

    function attachFieldGestures(f, node, pageEl) {
      let mode = null, lpTimer = null, start = null;
      const handle = node.querySelector('.handle');
      handle.addEventListener('pointerdown', (e) => {
        e.stopPropagation(); e.preventDefault(); mode = 'resize';
        const rect = pageEl.getBoundingClientRect();
        start = { px: e.clientX, py: e.clientY, w: f.w, h: f.h, rw: rect.width, rh: rect.height };
        handle.setPointerCapture(e.pointerId); selectField(f);
      });
      node.addEventListener('pointerdown', (e) => {
        if (e.target === handle || e.target.closest('.del')) return;
        e.stopPropagation(); selectField(f);
        const rect = pageEl.getBoundingClientRect();
      ...
      function endPointer() { clearTimeout(lpTimer); node.classList.remove('drag'); mode = null; start = null; }
    ...
    function autoFont(node) { const h = node.offsetHeight || 0; node.querySelector('.lbl').style.fontSize = Math.max(7, Math.min(h * 0.45, 26)) + 'px'; }
    function refreshFonts() { $$('.fld', stack).forEach(autoFont); }

    const pts = new Map(); let pinchStart = null;
    canvas.addEventListener('pointerdown', (e) => {
      if (e.target.closest('.fld')) return;
      pts.set(e.pointerId, e);
      if (pts.size === 2) { const [a, b] = [...pts.values()]; pinchStart = { dist: dist(a, b), scale }; }
    });
    canvas.addEventListener('pointermove', (e) => {
      if (!pts.has(e.pointerId)) return;
      pts.set(e.pointerId, e);
      if (pts.size === 2 && pinchStart) { const [a, b] = [...pts.values()]; setZoom(pinchStart.scale * (dist(a, b) / (pinchStart.dist || 1))); }
    });
  ...
  Screens.recipient = function (main) {
    ...
    const searchLabel = el('label', { class: 'field' }, [el('span', { text: t('rc_title') }), searchWrap]);
    ...
    function renderSuggest() {
      ...
      const matches = contacts().filter(c => (c.name || '').toLowerCase().includes(q) || (c.email || '').toLowerCase().includes(q) || (c.phone || '').toLowerCase().includes(q)).slice(0, 6);
      if (!matches.length) { suggest.style.display = 'none'; return; }
      matches.forEach(c => suggest.appendChild(el('div', { class: 'opt', onclick: () => pick(c), html: '<b>' + esc(c.name) + '</b><br><span class="small muted">' + esc(c.email || c.phone || '') + '</span>' })));
      suggest.style.display = 'block';
    }
    search.addEventListener('input', renderSuggest);

    main.appendChild(el('button', { class: 'btn subtle', style: { marginBottom: '12px' }, html: iconHTML('smartphone', 18) + t('rc_from_device'), onclick: pickFromDevice }));
    ...
    main.appendChild(el('details', { class: 'card' }, [
      el('summary', { html: iconHTML('plus', 18) + t('rc_add_new') }),
      el('div', { style: { marginTop: '12px' } }, [
        ...
        el('button', {
          class: 'btn subtle', html: iconHTML('save', 18) + t('save'),
          onclick: () => {
    ...
    function pick(c) { chosen = c; search.value = ''; suggest.style.display = 'none'; renderChip(); renderTail(); }
    function renderChip() {
      chipWrap.innerHTML = '';
      if (chosen) chipWrap.appendChild(el('div', { class: 'chip' }, [
        el('span', { html: iconHTML('user', 16) }),
        el('span', { text: chosen.name || chosen.email || chosen.phone }),
        el('button', { html: iconHTML('x', 15), onclick: () => { chosen = null; renderChip(); renderTail(); } })
      ]));
    }
    main.insertBefore(chipWrap, searchLabel);
    ...
    function renderTail() {
      ...
      const seg = el('div', { class: 'seg' }, [
        segBtn('email', 'mail', t('rc_via_email'), hasEmail),
        segBtn('sms', 'message', t('rc_via_sms'), hasPhone)
      ]);
      tail.appendChild(seg);
      tail.appendChild(el('button', { class: 'btn primary', html: iconHTML('send', 19) + t('rc_send'), onclick: doSend }));

      function segBtn(val, ic, label, enabled) {
        return el('button', {
          class: channel === val ? 'on' : '', disabled: enabled ? null : 'disabled',
          html: iconHTML(ic, 17) + label,
    ...
    function doSend() {
      ...
      sheet(el('div', { class: 'center' }, [
        el('div', { class: 'auth-head' }, [el('div', { class: 'glyph', html: iconHTML('send', 30) }), el('h3', { text: t('rc_sent_title') })]),
  ...
  Screens.sent = function (main) {
    main.appendChild(el('div', { class: 'ptitle' }, [el('h1', { text: t('sent_title') }), el('p', { text: t('sent_sub') })]));
    const docs = Store.get().sentDocs || [];
    if (!docs.length) { main.appendChild(window.emptyState('send', t('sent_empty'))); return; }
  ...
  function renderSentRow(d) {
    ...
    const pillIcon = d.status === 'signed' ? 'check' : (d.status === 'sent' ? 'send' : 'refresh');
    const pillText = d.status === 'signed' ? t('st_signed') : (d.status === 'sent' ? t('st_sent') : t('st_pending'));
    const ftype = (d.files && d.files[0] && d.files[0].type) || 'pdf';

    const actions = el('div', { class: 'actions' });
    actions.appendChild(el('button', { class: 'icon-btn', title: t('act_resend'), html: iconHTML('refresh', 18), onclick: () => toast(t('resent_ok')) }));
    if (d.status !== 'signed') actions.appendChild(el('button', { class: 'icon-btn', title: t('act_change'), html: iconHTML('user', 18), onclick: () => changeRecipient(d) }));
    actions.appendChild(el('button', { class: 'icon-btn', title: t('act_view'), html: iconHTML('eye', 18), onclick: () => { const f = d.files && d.files[0]; if (f) downloadBase64(f.b64, f.name, MIME[f.type]); } }));
    if (d.status === 'signed') {
      actions.appendChild(el('button', { class: 'icon-btn', title: t('act_dl'), html: iconHTML('download', 18), onclick: () => { const f = d.files && d.files[0]; if (f) downloadBase64(f.b64, 'signed_' + f.name, MIME[f.type]); } }));
    } else {
      actions.appendChild(el('button', { class: 'icon-btn', title: t('mark_signed'), html: iconHTML('check-circle', 18), onclick: () => markSigned(d) }));
    }

    return el('div', { class: 'row' }, [
      el('div', { class: 'ic file' }, [fileIcon(ftype, 38)]),
      el('div', { class: 'main' }, [
        el('div', { class: 't', text: d.name }),
        el('div', { class: 'sub', text: U.fmtDate(d.date) + ' · ' + (d.recipient.name || d.recipient.email || d.recipient.phone || '') })
      ]),
      el('span', { class: 'pill ' + pillClass, html: iconHTML(pillIcon, 12) + pillText }),
      actions
    ]);
  }

  function markSigned(d) { Store.update(st => { const x = st.sentDocs.find(s => s.id === d.id); if (x) x.status = 'signed'; }); toast(t('st_signed')); App.render(); }
  ...
  function changeRecipient(d) {
    ...
    const body = el('div', {}, [
      ...
      el('button', {
        class: 'btn primary', html: iconHTML('save', 18) + t('save'),
        onclick: () => {
          const val = contactIn.value.trim();
          Store.update(st => {
            const x = st.sentDocs.find(s => s.id === d.id);
            if (x) { x.recipient.name = nameIn.value.trim(); if (val.includes('@')) { x.recipient.email = val; x.recipient.channel = 'email'; } else { x.recipient.phone = val; x.recipient.channel = 'sms'; } }
          });
