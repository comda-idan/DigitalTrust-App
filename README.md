...
- **Passkey** — WebAuthn אמיתי: הרכשת מפתח גישה (`navigator.credentials.create`) במכשיר בעת ההרשמה, ואימות ביומטרי אמיתי מולו (`navigator.credentials.get` עם `allowCredentials`) בכל חתימה ובכניסה חוזרת. נפילה ל-PINCODE כשאין תמיכה.
